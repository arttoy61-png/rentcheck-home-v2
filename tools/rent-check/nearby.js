(() => {
  ...
  function render(info){
    ...
    const rows=info.rows.length?info.rows.map((r,i)=>`<tr><td><b>${i+1}</b></td><td>${Math.round(r.distance)}m${r.same?'<br><small>같은 지번</small>':''}</td><td>${esc(typeLabel(r.t))}<br><small>${esc(r.b||r.u)}</small></td><td>${num(r.a).toFixed(1).replace(/\.0$/,'')}㎡</td><td>${esc(price(r,kind))}<br><small>${esc(fmtDate(r))}</small></td></tr>`).join(''):'<tr><td colspan="5">조건에 맞는 주변 실거래를 찾지 못했습니다.</td></tr>';
    box.innerHTML=`<div class="verdict ${esc(v.cls)}"><div class="title">${esc(v.title)}</div><div class="sub">${esc(v.sub)}</div></div><div class="card nearby-card"><h2>입력 주소 주변 실거래</h2><div class="nearby-summary"><b>${esc(state.location.address)}</b><br>반경 500m · ${esc(info.typeInfo.label)} · 비슷한 면적 · 최근 ${info.period}개월</div><div id="nearbyMap" class="nearby-map" aria-label="주소 주변 실거래 지도"></div><div class="nearby-table-wrap"><table class="nearby-table"><thead><tr><th>#</th><th>거리</th><th>유형</th><th>면적</th><th>${kind==='전세'?'보증금':'보증금 / 월세'}</th></tr></thead><tbody>${rows}</tbody></table></div><p class="nearby-note">500m 안에서 조건이 비슷한 거래 ${info.total}건 중 최대 5건을 보여줍니다. 입력 주소는 카카오 위치검색에만 사용하며 Rent Check에 저장하지 않습니다.</p></div>`;
