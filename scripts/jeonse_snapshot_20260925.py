...
def finalize_copy(s):
    intro='<p class="lead">강서구 임대차의 전세·월세 계약 건수 변화를 비교한 과거 분석입니다. <strong>2026년 8월 23일 저장된 국토교통부 실거래 신고자료</strong>에서 2024년 9월~2026년 7월 23개월을 고정해 아파트, 주택(연립·다세대), 오피스텔을 따로 집계했습니다. 최신 시세를 실시간으로 보여주는 페이지는 아닙니다.</p>'
    s,n=re.subn(r'<p class="lead">.*?</p>',lambda m:intro,s,count=0,flags=re.S)
    assert n==1,n
    return s.replace('오피스텔는','오피스텔은')

def record_paths(names):
...
def apply():
    ...
    if MARK in old:
        corrected=finalize_copy(old)
        if corrected!=old:
            backup=OUT/'originals'/PAGE;backup.parent.mkdir(parents=True,exist_ok=True)
            if not backup.exists():backup.write_text(old,encoding='utf-8')
            (ROOT/PAGE).write_text(corrected,encoding='utf-8');record_paths([PAGE])
        verify();return
    data=snapshot();s=old
    ...
    for key,d in data['types'].items():
        ...
        particle='은' if key=='off' else '는'
        phrases.append(f'{d["label"]}{particle} 전세 {cf(jc)}, 월세 {cf(wc)}')
    ...
    record_paths([PAGE,DATA,'sitemap.xml'])
...
def verify():
    ...
    assert '2026년 8월 23일' in soup.select_one('.lead').get_text() and '오피스텔는' not in s
